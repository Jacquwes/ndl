...
function replaceChar() {
    ...
    if(Math.floor(Math.random() * 10) !== 0) {
        previousValue = document.querySelector('input').value;
        return;
    }
    let initialValue = document.querySelector('input').value;
    ...
    let nextChar = nextCharArray[Math.floor(Math.random() * nextCharArray.length)];
    
    initialValue = initialValue.substring(0, indice) + nextChar + initialValue.substring(indice + 1);
    previousValue = initialValue;

    document.querySelector('input').value = initialValue;
    document.querySelector('input').setSelectionRange(cursorPosition, cursorPosition);
}