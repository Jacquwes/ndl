function enablePing(duration) {
  document.getElementById("impossibleInput").addEventListener("keydown", addPing);
  setTimeout(() => {
    document.getElementById("impossibleInput").removeEventListener("keydown", addPing);
  }, duration);
}
...
async function packetLossLoop(id_) {
  if (buffer.length > 0) {
    ...
    let packet = buffer[0];
    setTimeout(() => {
      let input = document.getElementById("impossibleInput");
      let value = input.value;
      input.value =
        value.substring(0, packet.indice) +
        packet.char +
        value.substring(packet.indice + 1);
